...
impl<'a> HyperCall<'a> {
    ...
    pub fn hypercall(&mut self, code: u64, arg0: u64, arg1: u64) -> HyperCallResult {
        ...
        unsafe {
            match code {
                ...
                HyperCallCode::HvShmSignal => self.hv_shm_signal(arg0, arg1),
                _ => {
    ...
    fn hv_zone_list(&self, zones: *mut ZoneInfo, cnt: u64) -> HyperCallResult {
        ...
        /// Handle shared memory signal hypercall
    ...
    fn hv_shm_signal(&mut self, target_zone_id: u64, service_id: u64) -> HyperCallResult {
        ...
        // Check if the current zone is root zone (only root zone can send signals)
        if !is_this_root_zone() {
            return hv_result_err!(EPERM, "Only root zone can send shm signals");
        }

        // Find the target zone
        let target_zone = match find_zone(target_zone_id as usize) {
            Some(zone) => zone,
            None => {
                return hv_result_err!(ENOENT, "Target zone not found");
            }
        };

        // Get the target zone's CPU set
        let zone_read = target_zone.read();
        let target_cpus = zone_read.cpu_set.clone();
        drop(zone_read);
